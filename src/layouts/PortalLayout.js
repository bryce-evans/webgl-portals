...
class PortalLayout extends THREE.Group {
  ...
  getScenes() {
    return this.fold_portals(function(e, acc) {
      acc.push(e.scene()); return acc;
    }, []);
  }
  ...
  wireGeometry() {
    ...
    this.foreachPortal(function(p) {
      group.add(p.wireGeometry());
    });
    return group;
  }
  ...
  isFullyPlanar() {
    const planar = true;
    this.foreach_portal(function(p) {
      return planar && p.isPlanar();
    });
    return planar;
  }
  ...
  renderDebugUVs(show = true, container = undefined) {
    ...
    this.foreachPortal(function(p) {
      p.renderDebugUVs(show, container);
    });
  }
}
