...
})({"d1IDO":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "1ef74344f411421d";
"use strict";
...
},{}],"kNNFu":[function(require,module,exports) {
/**
...
var _gltfloaderJs = require("/modules/three.js/examples/jsm/loaders/GLTFLoader.js");
function main() {
    ...
    var camera = new _three.OrthographicCamera(width / -80, width / 80, height / 80, height / -80, 1, 1000);
    ...
    (0, _jqueryDefault.default)(document).keydown(function(event) {
        if (event.which == 32) {
            (0, _jqueryDefault.default)("#debug_uvs").show();
            show_debug_uvs = true;
        }
    });
    (0, _jqueryDefault.default)(document).keyup(function(event) {
        if (event.which == 32) {
            (0, _jqueryDefault.default)("#debug_uvs").hide();
    ...
    var buffer_scenes = [];
    var dummy_geos = [
        new _three.BoxGeometry(size, size, size),
        new _three.ConeGeometry(size, size, 6),
        new _three.DodecahedronGeometry(size / 2),
        new _three.IcosahedronGeometry(size / 2),
        new _three.TetrahedronGeometry(size / 2),
        new _three.TorusGeometry(size / 2, size / 4, 10, 10)
    ];
    var dummy_objs = [];
    ...
    var live_materials = [];
    for(var i = 0; i < 6; i++){
        var miniscene_renderer = new _three.WebGLRenderer({
            antialias: true
        });
        miniscene_renderer.setSize(width / 4, height / 4);
        debug_renderers.push(miniscene_renderer);
        var div = (0, _jqueryDefault.default)("<div>");
        div.append(miniscene_renderer.domElement);
        var canvas2d = (0, _jqueryDefault.default)(`<canvas height=${height / 4} width=${width / 4} class="overlay"></canvas>`);
        div.append(canvas2d);
        canvas2ds.push(canvas2d[0]);
        (0, _jqueryDefault.default)("#debug_uvs").append(div);
        var buffer_texture = new _three.WebGLRenderTarget(width, height, {
            minFilter: _three.LinearFilter,
            magFilter: _three.NearestFilter
        });
        buffer_textures.push(buffer_texture);
        var buffer_scene = new _three.Scene();
        buffer_scenes.push(buffer_scene);
        var light = new _three.PointLight(light_color, light_intensity);
        light.position.set(0, 3, 15);
        buffer_scene.add(light);
        buffer_scene.add(new _three.AmbientLight(0xfff));
        var hue = Math.random() * 360;
        var dummy_material = new _three.MeshPhongMaterial({
            color: new _three.Color("hsl(" + hue + ", 100%, 50%)")
        });
        var dummy_geometry = dummy_geos[i];
        var dummy_obj = new _three.Mesh(dummy_geometry, dummy_material);
        dummy_obj.position.z = -1;
        buffer_scene.add(dummy_obj);
        dummy_objs.push(dummy_obj);
        var room = new _three.BoxGeometry(10, 10, 10);
        var hue = Math.random() * 360;
        var dummy_bg_mat = new _three.MeshPhongMaterial({
            color: new _three.Color("hsl(" + hue + ", 100%, 50%)")
        });
        dummy_bg_mat.side = _three.BackSide;
        dummy_bg_materials.push(dummy_bg_mat);
        var dummy_bg = new _three.Mesh(room, dummy_bg_mat);
        dummy_bgs.push(dummy_bg);
        buffer_scene.add(dummy_bg);
        var live_material = new _three.MeshBasicMaterial({
            map: buffer_texture.texture
        });
    ...
    //  . -------------- .
    //
    buffer_scenes[0].setRotationFromEuler(new _three.Euler(0, Math.PI / 2, 0));
    buffer_scenes[1].setRotationFromEuler(new _three.Euler(0, -Math.PI / 2, 0));
    buffer_scenes[2].setRotationFromEuler(new _three.Euler(-Math.PI / 2, 0, 0));
    buffer_scenes[3].setRotationFromEuler(new _three.Euler(Math.PI / 2, 0, 0));
    buffer_scenes[4].setRotationFromEuler(new _three.Euler(0, 0, 0));
    buffer_scenes[5].setRotationFromEuler(new _three.Euler(Math.PI, 0, 0));
    // Forward render result to output texture.
    var mainBoxGeo = new _three.BoxGeometry(10, 10, 10);
    var mainBoxObject = new _three.Mesh(mainBoxGeo, live_materials);
    scene.add(mainBoxObject);
    function render() {
        controls.update();
        requestAnimationFrame(render);
        if (controls.show_debug_uvs) for(var i = 0; i < debug_renderers.length; i++){
        ...
        for(var i = 0; i < vertices.length / 3; i++){
            ...
            for(let j = 0; j < 3; j++){
                ...
                uvs.push({
                    x: projected.x * width / 4,
                    y: projected.y * height / 4
                });
                // Set the UVs.
                face_uvs[2 * j] = projected.x;
                face_uvs[2] = projected.y;
            }
            if (canvas) drawTriangle(canvas, uvs[0], uvs[1], uvs[2]);
        }
        ...
        for(var i = 0; i < 6; i++)debug_renderers[i].render(buffer_scenes[i], camera);
...
},{"/modules/three.js/examples/jsm/controls/OrbitControls.js":"f5yqU","/modules/three.js/examples/jsm/loaders/GLTFLoader.js":"cJwhw","three":"ktPTu","jquery":"hgMhh","@parcel/transformer-js/src/esmodule-helpers.js":"b9DC4"}],"cJwhw":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{"three":"ktPTu","../utils/BufferGeometryUtils.js":"1PP2n","@parcel/transformer-js/src/esmodule-helpers.js":"b9DC4"}],"1PP2n":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
},{"three":"ktPTu","@parcel/transformer-js/src/esmodule-helpers.js":"b9DC4"}],"hgMhh":[function(require,module,exports) {
/*!
...
},{}]},["d1IDO","kNNFu"], "kNNFu", "parcelRequire32f1")

//# sourceMappingURL=standalone_demo.f411421d.js.map
